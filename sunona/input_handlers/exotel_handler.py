"""
Sunona Voice AI - Exotel Input Handler

Handles incoming audio from Exotel streams.
"""
...
class ExotelInputHandler:
    """
    Exotel audio stream input handler.
    
    Processes incoming audio from Exotel calls.
    
    Example:
        ```python
        handler = ExotelInputHandler()
    ...
    async def process_message(self, message: str) -> Optional[bytes]:
        """
        Process a single WebSocket message from Exotel.
        
        Args:
            message: JSON message from Exotel
        ...
        try:
            ...
            if event == "start":
                ...
                logger.info(f"Exotel stream started: {self.stream_id}")
                ...
                logger.debug(f"Unknown Exotel event: {event}")
                return None
        
        except json.JSONDecodeError:
            logger.error("Invalid JSON from Exotel")
            return None
    ...
    async def process_stream(
        ...
        """
        Process incoming Exotel audio stream.
